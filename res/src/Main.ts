...
let size: number = Game.SIZE;
let scale: number = Math.min(width / size, height / size);
let positionX: number = (width - size * scale) / 2;
let positionY: number = (height - size * scale) / 2;
